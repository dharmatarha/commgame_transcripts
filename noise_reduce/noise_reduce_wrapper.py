"""
Script for reducing noise on CommGame audio recordings.

USAGE: python noise_reduce_wrapper.py PAIRNO [PAIRNO PAIRNO] --csv_path FILEPATH
...
- Noise wav files, one for each participant (that is, one for each (PAIRNO, LAB) combination). Noise files follow
the naming convention pairPAIRNO_LAB_noise.wav. These are usually generated by the noiseclip_generator.py utility.

Outputs are the noise-reduced versions of the *_repaired_mono.wav files,
following the naming convention pairPAIRNO_LAB_SESSION_repaired_mono_noisered.wav.
These are saved next to the *_repaired_mono.wav files.
...
def main():
    ...
    if not csv_path:
        warnings.war('As there was no csv file path provided, the same noise reduction ' 
    ...
    for pair in pair_numbers:
        for lab_name in lab_list:
            ...
            if csv_path:
                csv_index_pattern = 'pair' + str(pair) + '_' + lab_name + '_freeConv_audio.wav'  # to be used with *_audio.wav files
#                csv_index_pattern = 'pair' + str(pair) + '_' + lab_name + '_freeConv_repaired_mono.wav'  # to be used with *_repaired_mono.wav files
